...
const managedStart = "<!-- create-agent:managed:start -->";
const managedEnd = "<!-- create-agent:managed:end -->";
...
function upsertManagedBlock(existing, block) {
  ...
  if (start === -1 || end === -1 || end < start) throw new Error("Native instruction file contains an incomplete create-agent managed block");
...
async function atomicWrite(path, value) {
  ...
  const temporary = `${path}.create-agent-${process.pid}-${Date.now()}`;
...
export async function initializeEnvironment({
  ...
  const profile = {
    schemaVersion: "are-repository-profile/v1alpha1",
    generatedBy: `arenv/${generatorVersion}`,
  ...
  return {
    schemaVersion: "create-agent-init-result/v1alpha1",
    generatedAt: clock().toISOString(),
    dryRun,
    repoRoot,
    ...
    nextSteps: [
      ...
      "Start work with: arenv task start --objective \"...\" --agent codex",
      "Switch with: arenv task handoff --to claude --launch"
    ]
  };
}
